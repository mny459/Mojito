...
import com.mny.mojito.utils.CharacterHandler
...
class DefaultFormatPrinter : FormatPrinter {
    ...
    override fun printJsonResponse(
        ...
        bodyString =
            if (contentType != null && LogInterceptor.isJson(contentType)) CharacterHandler.jsonFormat(
                bodyString ?: ""
            ) else if (contentType != null && LogInterceptor.isXml(contentType)) CharacterHandler.xmlFormat(
                bodyString
            ) else bodyString
    ...
    companion object {
        ...
        private val MOJITO = arrayOf("-M-", "-O-", "-J-", "-I-", "-T-", "-0-")
        private fun computeKey(): String {
            if (last.get()!! >= MOJITO.size) {
                last.set(0)
            }
            val s = MOJITO[last.get()!!]
            last.set(last.get()!! + 1)
            return s
        }
